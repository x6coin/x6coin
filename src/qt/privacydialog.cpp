...
#include "zx6coincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zX6 ought to be enough for anybody." - Bill Gates, 2017
    ui->zX6payAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzX6SyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzX6->setEnabled(false);
        ui->pushButtonMintzX6->setToolTip(tr("zX6 is currently disabled due to maintenance."));

        ui->pushButtonSpendzX6->setEnabled(false);
        ui->pushButtonSpendzX6->setToolTip(tr("zX6 is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zX6payAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzX6_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zX6 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zX6...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zX6 in ") +
...
void PrivacyDialog::on_pushButtonSpendzX6_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zX6 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zX6
        sendzX6();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zX6
    sendzX6();
}

void PrivacyDialog::on_pushButtonZX6CoinControl_clicked()
{
    ZX6CoinControlDialog* zX6Control = new ZX6CoinControlDialog(this);
    zX6Control->setModel(walletModel);
    zX6Control->exec();
}

void PrivacyDialog::setZX6CoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzX6Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzX6()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid X6Coin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zX6payAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zX6 is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " X6COIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zX6</b>";
    ...
    // use mints from zX6 selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZX6CoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZX6CoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zx6coin selector in case it was used
    ZX6CoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zX6 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " X6Coin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zX6 Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zX6 </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zX6 "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zX6 "));
    ui->labelzX6AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzX6SyncStatus->setVisible(fShow);
}
