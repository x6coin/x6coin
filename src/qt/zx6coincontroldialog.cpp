...
#include "ui_zx6coincontroldialog.h"
...
std::list<std::string> ZX6CoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZX6CoinControlDialog::listMints;

ZX6CoinControlDialog::ZX6CoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZX6CoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZX6CoinControlDialog::~ZX6CoinControlDialog()
{
    delete ui;
}

void ZX6CoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZX6CoinControlDialog::updateList()
{
...
void ZX6CoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZX6CoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZX6Coin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZX6CoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZX6CoinControlDialog::GetSelectedMints()
{
...
void ZX6CoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
