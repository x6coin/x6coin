...
static bool AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("X6Coin Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                        "  x6coin-cli [options] <command> [params]  " + _("Send command to X6Coin Core") + "\n" +
                        "  x6coin-cli [options] help                " + _("List commands") + "\n" +
                        "  x6coin-cli [options] help <command>      " + _("Get help for a command") + "\n";
